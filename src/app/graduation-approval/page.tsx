...
function getApprovalStep(user: any) {
  ...
  if (user.userType === 1) {
    switch (user.staffRole) {
      case 3: return "departmentSecretary";
      case 2: return "facultyDeansOffice";
      case 1: return "studentAffairs";
      default: return null;
    }
  }
  return null;
...
function canApprove(process: any, step: string) {
  ...
  if (step === "departmentSecretary") return process.advisorApproved && !process.departmentSecretaryApproved;
  if (step === "facultyDeansOffice") return process.departmentSecretaryApproved && !process.facultyDeansOfficeApproved;
  if (step === "studentAffairs") return process.facultyDeansOfficeApproved && !process.studentAffairsApproved;
  return false;
}
...
export default function GraduationApprovalPage() {
  ...
  async function fetchDepartmentSecretaryStudents() {
    ...
    try {
      ...
      const studentsRes = await axiosInstance.get(`/students/by-department/${departmentId}`, {
        params: {
          pageIndex: 0,
          pageSize: 50
        }
      ...
      if (!studentsRes.data || (!Array.isArray(studentsRes.data.items) && !Array.isArray(studentsRes.data))) {
      ...
      setError(e.response?.data?.message || "Failed to fetch department students");
    }
    setLoading(false);
  }
  ...
  async function fetchFacultyDeansStudents() {
    ...
    try {
      ...
      const studentsRes = await axiosInstance.get(`/students/by-faculty/${facultyId}`, {
        params: {
          pageIndex: 0,
          pageSize: 50
        }
      ...
      // Check if we have items in the response
      if (!studentsRes.data || (!Array.isArray(studentsRes.data.items) && !Array.isArray(studentsRes.data))) {
        console.error("Unexpected faculty students response format:", studentsRes.data);
        setError("Invalid response format from server");
        setLoading(false);
        return;
      }
  ...
  async function handleApprove(processId: string, isApproved: boolean) {
    ...
    try {
      ...
      setProcesses((prev) => prev.map((p) => p.id === processId ? { ...p, [`${step}Approved`]: isApproved } : p));
    } catch (e) {
  ...
  const filteredProcesses = processes.filter((p) =>
    p.studentUser && `${p.studentUser.name} ${p.studentUser.surname}`.toLowerCase().includes(search.toLowerCase())
  );
  ...
  async function handleDepartmentSecretaryApproval(studentId: string, isApproved: boolean) {
    try {
      setLoading(true);
      await axiosInstance.post(`${BACKEND_URL}/approve-by-department-secretary`, {
        studentId,
        isApproved
      });
  ...
  async function handleFacultyDeansApproval(studentId: string, isApproved: boolean) {
    try {
      setLoading(true);
      await axiosInstance.post(`${BACKEND_URL}/approve-by-faculty-deans-office`, {
        studentId,
        isApproved
      });
      ...
      <AuthenticatedLayout>
        <div className="p-8 max-w-4xl mx-auto">
          <div className="flex items-center gap-4 mb-8">
            <h1 className="text-3xl font-bold text-gray-900">Advisor's Graduation List</h1>
          </div>
          <input
            type="text"
          ...
          {filteredStudents.length === 0 ? (
            ...
            <div className="space-y-4">
              {filteredStudents.map((s: any) => (
                <div key={s.id} className="bg-white rounded-xl shadow-md p-6 flex flex-col md:flex-row md:items-center md:justify-between border border-gray-100 hover:shadow-lg transition">
                  <div className="flex-grow">
                    <div className="font-semibold text-lg text-gray-900">{s.name} {s.surname}</div>
                    <div className="text-sm text-gray-600">Bölüm: <span className="text-gray-800">{s.departmentName || "-"}</span></div>
                    <div className="text-sm text-gray-600">Öğrenci No: <span className="text-gray-800">{s.studentNumber || "-"}</span></div>
                    <div className="mt-2">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        s.graduationStatus === 0 ? 'bg-yellow-100 text-yellow-800' : 
                        s.graduationStatus === 1 ? 'bg-blue-100 text-blue-800' :
                        s.graduationStatus === 2 ? 'bg-green-100 text-green-800' :
                        'bg-red-100 text-red-800'
                      }`}>
                        {s.graduationStatus === 0 ? 'Beklemede' : 
                         s.graduationStatus === 1 ? 'İşlemde' :
                         s.graduationStatus === 2 ? 'Onaylandı' : 
                         'Reddedildi'}
                      </span>
                    </div>
                  </div>
                  <div className="mt-4 md:mt-0 flex gap-2">
                    <button
                      ...
                      title={s.graduationStatus !== 0 ? 'Bu öğrencinin mezuniyet durumu zaten işlemde veya sonuçlandı' : ''}
                    >
                      Onayla
                    </button>
                    <button
  ...
  if (step === "departmentSecretary") {
    return (
      <AuthenticatedLayout>
        <div className="p-8 max-w-4xl mx-auto">
          <div className="flex items-center gap-4 mb-8">
            <h1 className="text-3xl font-bold text-gray-900">Department Secretary Student List</h1>
          </div>
          <input
            type="text"
            placeholder="Search by student name..."
                ...
                <div key={s.id} className="bg-white rounded-xl shadow-md p-6 flex flex-col md:flex-row md:items-center md:justify-between border border-gray-100 hover:shadow-lg transition">
                  ...
                  <div className="mt-4 md:mt-0 flex gap-2">
                    <button
                      ...
                      title={s.graduationStatus !== 1 ? 'Bu öğrencinin mezuniyet durumu henüz işleme hazır değil veya sonuçlandı' : ''}
                    >
                      Onayla
                    </button>
                    <button
  ...
  if (step === "studentAffairs") {
    return (
      <AuthenticatedLayout>
        <div className="p-8 max-w-4xl mx-auto">
          <div className="flex items-center gap-4 mb-8">
            <h1 className="text-3xl font-bold text-gray-900">Student Affairs Student List</h1>
          </div>
          <input
            type="text"
            placeholder="Search by student name..."
          ...
          {filteredStudents.length === 0 ? (
            ...
            <div className="space-y-4">
              {filteredStudents.map((s: any) => (
                <div key={s.id} className="bg-white rounded-xl shadow-md p-6 flex flex-col border border-gray-100 hover:shadow-lg transition">
                  ...
                  <div className="border-t border-gray-100 pt-4 mt-2">
                    <h3 className="text-sm font-semibold text-gray-900 mb-3">Onay Durumları</h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {/* Advisor Approval Status */}
                      <div className="flex items-center p-3 bg-gray-50 rounded-lg">
                        <div className={`w-3 h-3 rounded-full mr-3 ${
                          s.graduationStatus === 2 ? 'bg-green-500' : 
                          s.graduationStatus === 3 ? 'bg-red-500' : 
                          'bg-yellow-500'
                        }`} />
                        <div>
                          <div className="text-sm font-medium text-gray-900">Danışman</div>
                          <div className="text-sm font-medium" style={{ color: s.graduationStatus === 2 ? '#059669' : s.graduationStatus === 3 ? '#DC2626' : '#000000' }}>
                            {s.graduationStatus === 2 ? 'Onaylandı' : 
                             s.graduationStatus === 3 ? 'Reddedildi' : 
                             'Beklemede'}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center p-3 bg-gray-50 rounded-lg">
                        <div className={`w-3 h-3 rounded-full mr-3 ${
                          s.graduationStatus === 2 ? 'bg-green-500' :
                          s.graduationStatus === 3 ? 'bg-red-500' :
                          s.graduationStatus < 1 ? 'bg-gray-300' :
                          'bg-yellow-500'
                        }`} />
                        <div>
                          <div className="text-sm font-medium text-gray-900">Bölüm Sekreterliği</div>
                          <div className="text-sm font-medium" style={{ color: s.graduationStatus === 2 ? '#059669' : s.graduationStatus === 3 ? '#DC2626' : '#000000' }}>
                            {s.graduationStatus === 2 ? 'Onaylandı' :
                             s.graduationStatus === 3 ? 'Reddedildi' :
                             s.graduationStatus < 1 ? 'Danışman Onayı Bekleniyor' :
                             'Beklemede'}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center p-3 bg-gray-50 rounded-lg">
                        ...
                        <div>
                          <div className="text-sm font-medium text-gray-900">Fakülte Dekanlığı</div>
                          <div className="text-sm font-medium" style={{ color: s.graduationStatus === 2 ? '#059669' : s.graduationStatus === 3 ? '#DC2626' : '#000000' }}>
                            {s.graduationStatus === 2 ? 'Onaylandı' :
                             s.graduationStatus === 3 ? 'Reddedildi' :
                             s.graduationStatus < 1 ? 'Önceki Onaylar Bekleniyor' :
                             'Beklemede'}
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="mt-4 flex justify-end gap-2">
                    <button
                      ...
                      title={s.graduationStatus !== 1 ? 'Bu öğrencinin tüm onayları tamamlanmadan işlem yapamazsınız' : ''}
                    >
                      Onayla
                    </button>
                    <button
  ...
  if (step === "facultyDeansOffice") {
    return (
      <AuthenticatedLayout>
        <div className="p-8 max-w-4xl mx-auto">
          <div className="flex items-center gap-4 mb-8">
            <h1 className="text-3xl font-bold text-gray-900">Faculty Dean's Office Student List</h1>
          </div>
          <input
            type="text"
            placeholder="Search by student name..."
